...
var configSchema = exports_external.object({
  fhirBaseUrl: exports_external.string(),
  enclaveSharedSecret: exports_external.string(),
  policyRegistryAddress: exports_external.string(),
  claimRequestAddress: exports_external.string(),
  claimSettlementAddress: exports_external.string(),
...
function hexToBytes2(hex) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2++) {
    bytes[i2] = parseInt(h.slice(i2 * 2, i2 * 2 + 2), 16);
  }
  return bytes;
}
function bytesToHexStr(bytes) {
  let out = "0x";
  for (let i2 = 0;i2 < bytes.length; i2++) {
    out += HEX[bytes[i2] >> 4] + HEX[bytes[i2] & 15];
  }
  return out;
}
...
function hexToBase642(hexStr) {
  const clean = hexStr.startsWith("0x") ? hexStr.slice(2) : hexStr;
  return bytesToBase64(hexToBytes2(clean.padStart(40, "0").slice(-40)));
}
function topicToBase64(hexStr) {
  const clean = hexStr.startsWith("0x") ? hexStr.slice(2) : hexStr;
  return bytesToBase64(hexToBytes2(clean.padStart(64, "0")));
}
...
function decryptPayload(encryptedHex, sharedSecret) {
  const encBytes = hexToBytes2(encryptedHex);
  const keyBytes = hexToBytes2(sharedSecret);
  const decrypted = new Uint8Array(encBytes.length);
  for (let i2 = 0;i2 < encBytes.length; i2++) {
    decrypted[i2] = encBytes[i2] ^ keyBytes[i2 % keyBytes.length];
  }
  return utf8Decode(decrypted);
}
var COVERED_ICD10_CODES = new Set([
  "J06.9",
  "J18.9",
  "M54.5",
  "K21.0",
...
var REIMBURSEMENT_RATE = 0.8;
var MAX_PAYOUT_USD = 500;
function evaluateMedicalClaim(fhir, coverageStart, coverageEnd) {
  ...
  if (!icd10 || !treatmentDateStr || billedAmount <= 0) {
    return { status: "escalated", payoutAmount: 0, reasonCode: 6 };
  }
  if (!COVERED_ICD10_CODES.has(icd10)) {
    return { status: "denied", payoutAmount: 0, reasonCode: 1 };
  }
  const treatmentTs = Math.floor(new Date(treatmentDateStr).getTime() / 1000);
  if (treatmentTs < coverageStart || treatmentTs > coverageEnd) {
    return { status: "denied", payoutAmount: 0, reasonCode: 2 };
  }
  const rawPayout = billedAmount * REIMBURSEMENT_RATE;
  const cappedPayout = Math.min(rawPayout, MAX_PAYOUT_USD);
  return { status: "approved", payoutAmount: Math.floor(cappedPayout * 1e6), reasonCode: 0 };
}
function decodeLogData(data) {
  let timestamp = 0;
  for (let i2 = 58;i2 < 64; i2++)
    timestamp = timestamp * 256 + data[i2];
  let byteLen = 0;
  for (let i2 = 92;i2 < 96; i2++)
    byteLen = byteLen * 256 + data[i2];
  return { encryptedPayloadHex: bytesToHexStr(data.slice(96, 96 + byteLen)), timestamp };
}
var onClaimSubmitted = (runtime2, log) => {
  ...
  runtime2.log("[CRE ENCLAVE] ClaimSubmitted event received");
  runtime2.log(`[CRE ENCLAVE] Policy: ${policyIdHex}`);
  runtime2.log("[CRE ENCLAVE] FHIR claim ID: [PRIVATE — stays in TEE]");
  ...
  runtime2.log("[CRE ENCLAVE] World ID verification PASSED — human identity confirmed");
  runtime2.log("[CRE ENCLAVE] Step 2: Fetching FHIR record via ConfidentialHTTPClient...");
  const fhirUrl = `${config.fhirBaseUrl}/Claim/${bundle.fhirId}`;
  const fhirResp = confHTTP.sendRequest(runtime2, {
    request: {
      url: fhirUrl,
      method: "GET",
      multiHeaders: {
  ...
  if (!ok(fhirResp)) {
    runtime2.log("[CRE ENCLAVE] FHIR fetch failed");
    return {
      policyId: policyIdHex,
      claimant,
      status: "escalated",
  ...
  const fhirRecord = JSON.parse(utf8Decode(fhirResp.body));
  if (fhirRecord.resourceType !== "Claim") {
    runtime2.log("[CRE ENCLAVE] ERROR: Unexpected FHIR resource type");
    return {
      policyId: policyIdHex,
      claimant,
      status: "escalated",
  ...
  runtime2.log("[CRE ENCLAVE] FHIR record fetched and parsed inside TEE (not logged externally)");
  runtime2.log("[CRE ENCLAVE] Step 3: Running eligibility logic inside TEE...");
  ...
  for (let i2 = 0;i2 < hashInput.length && complianceHash.length < 66; i2++) {
    const code2 = hashInput.charCodeAt(i2);
    complianceHash += HEX[code2 >> 4] + HEX[code2 & 15];
  }
  complianceHash = complianceHash.padEnd(66, "0").slice(0, 66);
  runtime2.log(`[CRE ENCLAVE] Verdict: ${verdict.status.toUpperCase()} — $${(verdict.payoutAmount / 1e6).toFixed(2)} USDC`);
  runtime2.log("[CRE ENCLAVE] Diagnosis, ICD-10 code, amounts remain private in TEE");
  const verdictResult = {
    policyId: policyIdHex,
    claimant,
  ...
  try {
    const evmClient = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-mainnet-base-1"]);
    const calldata = encodeRecordVerdict(policyIdHex, verdict.status, verdict.payoutAmount, verdict.reasonCode, complianceHash);
    evmClient.writeReport(runtime2, {
      receiver: config.policyRegistryAddress,
      report: new Report({ rawReport: bytesToBase64(calldata) })
    }).result();
    runtime2.log("[CRE ENCLAVE] Step 5: Verdict written onchain via WriteTarget");
  } catch (e) {
    runtime2.log(`[CRE ENCLAVE] Step 5: WriteReport skipped (${e instanceof Error ? e.message : "simulation mode"})`);
  }
  return verdictResult;
};
function encodeRecordVerdict(policyId, status, payoutAmount, reasonCode, complianceHash) {
  ...
  for (let i2 = 0;i2 < encoded.length; i2++)
    bytes[i2] = encoded.charCodeAt(i2);
  return bytes;
}
var initWorkflow = (config) => {
  const evmClient = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-mainnet-base-1"]);
  const logTrigger = evmClient.logTrigger({
    addresses: [hexToBase642(config.claimRequestAddress)],
    topics: [{
